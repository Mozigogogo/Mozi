...
| `ROBOT_BACKEND_URL` | 可选；Python Robot 后端根地址，默认 `https://askmozi.com`；`/bigorder` 默认请求 `{ROBOT_BACKEND_URL}/bigorder/v1/chat` |
...
| `MOZI_APP_ENV` / `NEXT_PUBLIC_APP_ENV` | 可选；写入登录 body 的 `env`（与前端一致），未设时默认 `production` |
| `TG_REGISTER_LOG` | 可选；新用户注册流程日志（`[tg/login]`），默认开启；`0` 关闭 |
| `TG_REGISTER_DEBUG` | 可选；设为 `1` 时打印更详细的注册调试信息 |
