...
export default function PriceRankPage() {
  const router = useRouter();
  const { t } = useTranslation();
  const { shareCount, incrementShareCount } = useShareCount('pricechange');
  ...
  const gridTitle = useMemo(() => [t('priceRank.symbol'), t('priceRank.gainers')], [t]);

  const fetchData = async (dim) => {
    setLoading(true);
    setError(false);
    try {
      const res = await request({ url: Interface.price_change, data: { dim } });
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerContent}>
          <div className={styles.left}>
            <div className={styles.title}>{t('home.rank.up')}</div>
