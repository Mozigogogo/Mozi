...
export default function UpTradeRankPage() {
  ...
  const initialIntervals = (searchParams && searchParams.get('intervals')) || '1_day';
  const allowed = new Set(["1_day","7_day","1_month"]);
  ...
  const gridTitle = useMemo(() => ["币种", "成交额"], []);

  const fetchData = async (intervals) => {
    setLoading(true);
    setError(false);
    try {
      const res = await request({ url: Interface.PRICE_UPTRADE, data: { intervals } });
  ...
  const onShare = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: '飙升榜', text: '实时飙升榜', url: window.location.href });
      } else {
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerContent}>
          <div className={styles.left}>
            <div className={styles.title}>飙升榜</div>
