...
export default function TradeRankPage() {
  const router = useRouter();
  const { t } = useTranslation();
  const { shareCount, incrementShareCount } = useShareCount('trade');
  ...
  const gridTitle = useMemo(() => [t('priceRank.symbol'), t('priceRank.turnover')], [t]);

  const fetchData = async (intervals) => {
    setLoading(true);
    setError(false);
    try {
      const res = await request({ url: Interface.coin_trade, data: { intervals } });
  ...
  const onShare = () => {
    ...
    const shareText = encodeURIComponent(`${t('home.rank.volume')} - ${t('exchangeRank.realTimeUpdate')}`);
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerContent}>
          <div className={styles.left}>
            <div className={styles.title}>{t('home.rank.volume')}</div>
          ...
          <div className={styles.actionsCapsule}>
            <div className={styles.capsuleBtn} onClick={() => router.push(`/rankdiscuss?type=trade&name=${encodeURIComponent(t('home.rank.volume'))}`)}>
