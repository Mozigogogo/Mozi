  ...
  useEffect(() => {
    ...
    const nodes = container
      ...
      .html(d => {
        ...
        // 首页专用：根据块的实际大小动态调整字号
        // 大块用大字号，小块用小字号，最小字号不低于 10px
        const areaRatio = area / (width * height); // 块占总面积的比例
        
        // 名称字号：根据块的宽度和高度计算，大块最大 18px，小块最小 10px
        ...
        // 数值字号：比名称稍大，大块最大 18px，小块最小 10px
        const valueFontSize = Math.max(10, Math.min(
          itemWidth / 6,    // 基于宽度
          itemHeight / 3,   // 基于高度
          16                // 最大字号限制为 18px
        ));
